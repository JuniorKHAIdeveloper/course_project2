describe("Rooms Page", () => {
  ...
  it("create device successfully", () => {
    cy.visit("http://localhost:3000/dashboard/rooms");
    cy.get("#new-room").click();
    cy.get("body").contains("Create new room");
    cy.get('[name="roomName"]').type("Room 1");
    cy.get('[type="submit"]').click();
    cy.get("body").contains("Room created!");
    cy.get("body").contains("Room 1");
  });
  ...
  it("add device to room sucessfully", () => {
    ...
    cy.get('[name="Device 1"]').click();
    cy.get('[type="submit"]').click();
    cy.get("body").contains("Devices added!");
    cy.get("body").contains("Device 1");
  });
});
